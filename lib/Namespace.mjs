      ...
      Promise.allSettled(promises)
        .then(() => {
          if(Object.keys(this.#failed).length) {
            Log.warning({ message: `Namespace failed loading some modules: ${JSON.stringify(Object.keys(this.failed))}`, class: this.name });
          } else {
            Log.success({ message: `Namespace completed loading modules`, class: this.name });
          }
          resolve(this.loaded);
        });
    });
  ...
  static autoRegister(modulePath = Namespace.default.modulePath, basePath = this.basePath) {
    return new Promise((resolve, reject) => {
      Namespace.registerPath(modulePath, basePath)
        .then((registered) => {
          Log.success({ message: `Namespace completed registering modules`, class: this.name });
          resolve(this.registered);
        }).catch((error) => {
          Log.error({ message: error, class: this.name });
          reject(error);
        });
    });
  }
