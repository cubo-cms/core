...
/** @module Namespace
  ...
  *   NOTE: This class can publish loaded modules globally
  **/
export default class Namespace {
  /** @property {object} default - holds default values
    **/
  static default = {
    publishGlobally: true,                      // Publish modules as global
  ...
  /** Static @function autoLoad(registry)
    ...
    * @return {object}
    **/
  static autoLoad(registry = undefined) {
    return new Promise((resolve, reject) => {
      ...
      Promise.allSettled(promises).then(() => {
        if(promises.filter(promise => promise.status === 'rejected').length)
          Log.warning({ message: `Namespace failed to load some modules`});
        else
          Log.success({ message: `Namespace completed loading modules` });
        resolve(Namespace.loaded);
      });
    });
  }
  ...
  static autoRegister(modulePath = Namespace.default.modulePath, basePath = this.basePath) {
    return new Promise((resolve, reject) => {
      Namespace.registerPath(modulePath, basePath)
        .then((registered) => {
          Log.success(`Namespace completed autoregistering modules`);
  ...
  /** Static asynchronous @function load(moduleName)
    *
  ...
  static async load(moduleName) {
    try {
      if(Namespace.isLoaded(moduleName)) {
        throw new FrameworkError({ message: `Namespace claims module \"${moduleName}\" was already loaded`, type: 'warning' });
        ...
        module = await import('./' + path.relative(this.Path, registration.path));
        ...
        throw new FrameworkError({ message: `Namespace claims module \"${moduleName}\" is not registered`, type: 'error' });
      }
    } catch(error) {
      new Log(error);
    }
  ...
  static registerPath(modulePath = this.default.modulePath, basePath = this.basePath, dependency = undefined) {
    return new Promise((resolve, reject) => {
      ...
      fs.readdir(relPath, { 'encoding': 'utf8', 'withFiletypes': true }, (error, files) => {
        ...
        Promise.allSettled(promises).then(() => {
          if(promises.filter(promise => promise.status === 'rejected').length)
            Log.warning({ message: `Namespace failed to scan some folders`});
          else
            Log.success({ message: `Namespace completed scanning folders` });
          resolve(this.registered);
        });
      });
    });
