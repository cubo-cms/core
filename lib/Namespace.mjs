...
export default class Namespace {
  ...
  static async autoLoad(registry = undefined) {
    try {
      ...
      Log.success({ message: `Namespace completed autoloading modules` });
      return Namespace.loaded;
    } catch(error) {
      Log.error({ message: `Namespace failed to autoload modules` });
    }
  }
  /** Static @function autoRegister(path,dependency)
    ...
    * @return {object}
    **/
  static autoRegister(modulePath = Namespace.default.modulePath, basePath = '.') {
    if(Namespace.registerPath(modulePath, basePath))
      Log.success(`Namespace completed autoregistering modules`);
    else
      Log.error(`Namespace failed to autoregister modules`);
  ...
  static async load(moduleName) {
    try {
      if(Namespace.isLoaded(moduleName)) {
        ...
        module = await import('./' + path.relative(Core.thisPath, registration.path));
  ...
  /** Static @function registerPath(path,dependency)
    *
    ...
    * @return {object}
    **/
  static registerPath(modulePath, basePath, dependency = undefined) {
    try {
      ...
      for(const file of files) {
        if(fs.lstatSync(path.join(relPath, file)).isDirectory()) {
          Namespace.registerPath(path.join(modulePath, file), basePath, file)
        } else {
