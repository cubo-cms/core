...
export default class Core {
  ...
  /** Static @function basePath()
    ...
    **/
  static get basePath() {
    return Core.thisPath.substring(0, Core.thisPath.indexOf(path.delimiter + 'node_modules') == -1 ? Core.thisPath.lastIndexOf(path.delimiter) : Core.thisPath.indexOf(path.delimiter + 'node_modules'));
  }
    ...
    if(typeof data === 'string') {
      switch(path.extname(data)) {
        case '.json':
          ...
          break;
        default:
          return Core.loadFile(data, defaultData);
      }
    } else {
      return Core.loadJSON(data, defaultData);
    }
